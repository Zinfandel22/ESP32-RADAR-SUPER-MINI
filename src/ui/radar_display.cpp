...
namespace ui
{
  namespace radar
  {
    ...
    uint16_t kColorSweep = 0x07E0; // Bright green for sweep line
  ...
  namespace
  {
    ...
    unsigned long s_last_anim_ms = 0;
    ...
    void drawSweepLine(LGFX_Sprite &gfx, float angle_deg)
    {
      // 0 degrees is East in standard trig. Subtract 90 to make 0 degrees = North.
      float angle_rad = (angle_deg - 90.0f) * (M_PI / 180.0f);

      // Assuming a 240x240 display based on config.h
      int cx = 120;
      int cy = 120;
      int radius = 120;

      int end_x = cx + (radius * cos(angle_rad));
      int end_y = cy + (radius * sin(angle_rad));

      // Draw a bright line. You can use radar::kColorSweep or a bright green (0x07E0)
      gfx.drawLine(cx, cy, end_x, end_y, radar::kColorSweep);
    }
    ...
    void renderFrame()
    {
      drawStaticGrid(s_frame); // opens its own DrawScope(s_frame)
      {
        ...
        drawSweepLine(s_frame, s_sweep_angle);
        drawAircraft();
      }
  ...
  {
    // 1. Calculate time delta for smooth animation
    unsigned long now = millis();
    if (s_last_anim_ms == 0)
    {
      s_last_anim_ms = now;
    }
    unsigned long dt = now - s_last_anim_ms;
    s_last_anim_ms = now;

    // --- NEW CAPTURE LOGIC ---
    // If dt is huge (e.g., > 200ms), the network was blocking us.
    // Cap the time step so the line doesn't skip or teleport.
    if (dt > 200)
    {
      dt = 50; // Advance by a nominal 50ms frame so it resumes smoothly
    }

    // 2. Advance the angle (10 seconds per 360 degrees = 0.036 degrees per ms)
    s_sweep_angle += 0.036f * dt;

    // 3. Wrap around and trigger ADSB fetch when passing North (360 degrees)
    if (s_sweep_angle >= 360.0f)
    {
      s_sweep_angle -= 360.0f;
      services::adsb::requestRefresh();
    }

    // 4. Render the frame with the updated angle
    if (ensureFrameSprite())
    {
      renderFrame();
    }
